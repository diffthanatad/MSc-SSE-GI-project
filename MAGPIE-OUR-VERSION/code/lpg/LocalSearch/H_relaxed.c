  ...
  *num_Timed_Prec = 0;
  ...
  while (!fixpoint) {
	...
	continue;
      ...
      // Se so gi� che nn esiste un intervallo per questa precondizione, la salto
...
float get_cost_for_future_possibilities(int action, int level, float action_start_time, float action_end_time) {
  ...
  for (tmd = 0; tmd < gnum_timed_facts; tmd++) {
	...
	duration = MAX(0, (gtimed_fct_vect[tmd][j].end_time - action_end_time));
      ...
      // Valuto quante volte l'azione pu� essere eseguita nel tempo stimato 
   ...
   *****/
   ...
   switch (gcomp_var[fact_pos].l_operator)
	...
	break;
       ...
       MSG_ERROR("Warning: Errore nel calcolo dei valori di raggiungibilit�, la variabile non � una precondizione");
      break;
    } 
  ...
  if(GpG.input_plan==TRUE && GpG.weight_input_plan_cost)
    {
      ...
      if(Hvar.input_plan_actions[pos]<=0)
	Hvar.input_plan_cost++;
      else
	if(GpG.adapt_all_diff)
	  Hvar.input_plan_cost--;
	      
      Hvar.input_plan_actions[pos]--;
    }
 /* ADAPT */
  ...
  */
  ...
  for (i = 0; i < gef_conn[action].num_numeric_effects; i++)
      ...
      for (tmp = gcomp_var[var].affects; tmp; tmp = tmp->next)
	{
	  ...
	  //se la variabile � w_is_goal 
	  if (vectlevel[level]->numeric->w_is_goal[toCheck] > 0)
	    {

	      //se la precondizione non � soddisfatta la salto
	      if (!is_num_prec_satisfied(toCheck, level))
		continue;

	      //se � soddisfatta controllo che lo resti dopo l'applicazione dell'azione
                           ...
                           float max_time_for_timed_fact, node_cost_list max_n_cost)
{
  ...
  if (prec_par)
    {
	  ...
	  for (i = 0, temp = 0.0; i < gef_conn[act_pos].num_PC; i++)
		...
		{ 
		  ...
		  if(loc_dg_cost->best_act== INITIAL_ACTION && n_cost->weight ==0 )
		      n_cost->weight =1.0;
		  
		  if(loc_dg_cost->best_act== INITIAL_ACTION && n_cost->num_actions ==0 )
		      n_cost->num_actions =1.0;
		  
		  if(loc_dg_cost->best_act== INITIAL_ACTION && n_cost->act_time ==0 )
		      n_cost->act_time =1.0;
  ...
  /* ADAPT cost */
  if(GpG.input_plan==TRUE && Hvar.input_plan_actions[act_pos]<=0 && GpG.weight_input_plan_cost)
...
// E' abilitato GpG.extended_unsupported_facts e l'elemento del vicinato considerato non e' l'azione che ha come precondizione l'inconsistenza stessa (per cui si valuta la rimozione)
  ...
  if (GpG.num_solutions && neighb_act->constraint_type == C_T_INSERT_ACTION)
    {
      if (GpG.orig_weight_cost > 0 && GpG.best_exec_cost * 1.5 > GpG.total_cost)
	{
	  //  printf("-");
	  neighb_act->cost.act_cost += get_action_cost (neighb_act->act_pos, neighb_act->act_level, NULL);
...
float compute_relaxed_fact_cost (int Fact_position, int level, node_cost_list n_cost, int action_level, float max_time_for_timed_fact)
{
  ...
  **/
  ...
  /* ADAPT cost */
	if(GpG.input_plan==TRUE && GpG.weight_input_plan_cost)
	  if(times>0)
	  {
	    if( Hvar.input_plan_actions[best_action]< times)
	      {
		if( GpG.adapt_all_diff )
		  Hvar.input_plan_cost-=Hvar.input_plan_actions[best_action];

		  Hvar.input_plan_cost+=(times-1-Hvar.input_plan_actions[best_action] );
	      ...
	      Hvar.input_plan_actions[best_action]-=(times-1);
  ...
  **/
  ...
  if ( Hvar.action!=NULL && (Hvar.action->constraint_type == C_T_REMOVE_ACTION) && (best_action == Hvar.action->act_pos))
    {
       Hvar.weight_facts_define_cost+= 2;
       n_cost->weight+= 2;
        if(DEBUG4)
	  printf ("\n constraint_type == C_T_REMOVE_ACTION &&  best_action == Hvar.action->act_pos; break.");
  ...
  if (temp == 0)	// inserisco subito best action per evitare cicli di inserimento
    {
      Hvar.weight_facts_define_cost++;
      if(DEBUG4)
	printf("\n Best action already considered; break.");
  ...
  for (j = 0, k = 0, prec = 0.0;
      ...
      if ((el >= 0 && gft_conn[el].level>=0)  ||  CHECK_FACT_POS (el, best_level))
	{
	  ...
	  */
	  ...
	  if(DEBUG4)
	    if (best_act_type == IS_ACTION)
	      printf ("\n\n  EVALUATE  BEST_ACTION precondition,   %s  time %d pos %d ",
	    print_op_name_string (best_action, temp_name), best_level,
	    best_action);
	  ...
	  if (CHECK_FACT_POS (el, best_level))
		...
		Azzero loc_n_cost
	      */
	      ...
	      if(DEBUG4)
		if (best_act_type == IS_ACTION)
		  printf ("\n\n  EVALUATE  BEST_ACTION precondition overall,   %s  time %d pos %d ",
			  print_op_name_string (best_action, temp_name), best_level,
			  best_action);
	  ...
	  if (CHECK_FACT_POS (el, best_level))
		...
		Azzero loc_n_cost
	      */
	      ...
	      if(DEBUG4)
		if (best_act_type == IS_ACTION)
		  printf ("\n\n  EVALUATE  BEST_ACTION precondition at end,   %s  time %d pos %d ",
			  print_op_name_string (best_action, temp_name), best_level,
			  best_action);
