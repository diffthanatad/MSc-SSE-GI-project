...
void update_precond()
{
  ...
  **/
  for (false_fa_pos = 0; false_fa_pos < GpG.num_false_fa; false_fa_pos++) {
	 ...
	 /**
	    Se la precondizione � nell'ultimo livello, significa che � una precondizione di un goal,quindi 
	 ...
	 **/
	 ...
	 /**
	    ...
	    aggiornato il moltplicatore dell'azione corrispondente a quel livello, sempre se tale azione � 
	    in uso
	    **
	 ...
	 **/
         /**
	    controlla se l'azione che richiede la precondizione � in uso nel piano
	    **
	 ...
	 **/
         if (GET_ACTION_OF_LEVEL (*inconsistenza->level)->w_is_used){ 
               ...
               /**
		  *inconsistenza->level � il livello nel quale la precondizione non � supportata
		  inconsistenza->action � l'azione che ha tale precondizione
		  **
  ...
  for (false_fa_pos = 0; false_fa_pos < GpG.num_false_fa; false_fa_pos++){
	 ...
	 **
      ...
      **/
      ...
      /**
	 Nel caso in cui la precondizione si trovi in un livello sottostante a quello dei goals viene 
	 aggiornato il moltplicatore dell'azione corrispondente a quel livello, sempre se tale azione � in 
	 uso
	 **
      ...
      **/
      if (GET_ACTION_OF_LEVEL (*inconsistenza->level)->w_is_used){ //controlla se l'azione che richiede la precondizione � in uso nel piano
  ...
  **/
  ...
  switch (gcomp_var[index].l_operator)
    ...
    case EQUAL_OP:
      ...
      // se il valore di first_op � minore di quello di second_op faccio vicinato come per GREATER_OR_EQUAL_OP
  ...
  **/
  ...
  for (indlevel = get_prev(level); indlevel >= 0; indlevel = get_prev(indlevel)) {
    ...
    switch (gcomp_var[fct_pos].l_operator) 
      ...
      case EQUAL_OP:
	// MODIFICATO X UGUALIANZA
	// se l'azione modifica il valore della variabile pi� di MAX_APPROX allora la metto nel vicinato x rimozione
...
constraints_list choose_random_inconsistence()
{
  /* Questa funzione seleziona una inconsistenza in modo casuale da uno dei
     ...
     La probabilit� di selezione di uno dei tipi viene fatta variare  
  ...
  **/
  for (i=0;i<GpG.num_false_num_fa;i++)
    ...
    **/
    ...
    /**
       Una volta trovate le inconsistenze presenti al livello pi� basso, casualmente se ne seleziona una
       **
 ...
 * Scopo: Questa funzione seleziona una inconsistenza scegliendone una casuale tra quelle che possiedono  il costo pi� basso
 * Tipo: constraints_list
 * Input:
 * Output:
 * Strutture dati principali:
...
constraints_list choose_min_level_cost_inconsistence()
{
  ...
  **/
  for (i=0;i<GpG.num_false_act;i++)
    ...
    /**
       Una volta trovate le inconsistenze presenti al livello pi� basso, casualmente si seleziona la meno costosa
       **
...
constraints_list choose_min_level_constr_inconsistence()
{
  ...
  **/
  for (i=0;i<GpG.num_false_act;i++)
    ...
    /**
       Una volta trovate le inconsistenze presenti al livello pi� basso, si seleziona la meno costosa
       **
       When the lower level inconsistence have been found, I select the less expensive
    **/
   /**
...
int search_step(int num_try, int min_time, int numrestart, PlanAction ** plan_actions, State * start_state, State * end_state)
{
  ...
  /** 
      ...
      state trovate gi� altre soluzioni.
  ...
  **/
  ...
  do
      ...
      for (num_restart = 0; num_restart < GpG.numrestart; num_restart++)
	{
	  ...
	  /**
	     Questo � il numero totale dei fatti e delle azioni che erano false
	     **
  ...
  if (num_run >= GpG.numrun) {
    printf(" Total restart limit reached, exiting.\n\n");
  }
