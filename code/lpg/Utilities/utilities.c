 ...
 * AAA     Via Branze 38, 25123 Brescia, Italy
 *
...
VALLATI,
**/
 GpG.numerator_coef=1.5;
 GpG.tabu_length_coef=1.2;
 GpG.uns_vect = 25;
...
int set_param (int num_unsat)  
{
   ...
   if (position < (GpG.uns_vect-1) )
   ...
   else
    {
      ...
      for (i = 1; i < GpG.uns_vect; i++)
	mean += unsat_vector[i];
      mean /= (GpG.uns_vect);

      var = 0.0;
      for (i = 0; i < GpG.uns_vect; i++)
	{
      ...
      else
	{
	  if(GpG.numerator<GpG.max_numerator)
	    {
/** 
VALLATI, numerator_coef era a 1.5, tabu_length_coef a 1.2
**/
	      GpG.numerator = (int) (GpG.numerator *  GpG.numerator_coef);
	      GpG.tabu_length =(int) (GpG.tabu_length * GpG.tabu_length_coef);
	    }
	  /**
...
/**
VALLA
Il valore iniziale era 1.25
**/
      if (GpG.static_noise == FALSE && (GpG.init_numerator * GpG.noise_increment < 40) )
	{
	  GpG.init_numerator =(int) (GpG.init_numerator * GpG.noise_increment);
	}
    }
  else
...
else if (strcmp (argv[i], "-triomemory") == 0)
	sscanf (argv[++i], "%d", &GpG.lowmemory); // 0, 1, 2; raccoglie i diversi casi!
...
/* VALLATI, ichoice attivare o meno.
*/
	else if (strcmp (argv[i], "-initializeichoice") == 0) 
	sscanf (argv[++i], "%d", &GpG.initialize_inc_choice);

      //MODIFICHE COCCOLI e VALLATI
      else if (strcmp (argv[i], "-nonuniform_random") == 0)
	sscanf (argv[++i], "%d", &GpG.nonuniform_random_incosistence_test);
...
/* VALLATI sui 3 bestfirst: 0 = nobestfirst, 1 = bestfirst, 2 = onlybestfirst
*/ 

      else if (strcmp (argv[i], "-bestfirst") == 0)
	{
		sscanf (argv[++i], "%d", &aiuto);
		if (aiuto == 0 || aiuto == 1 )
			GpG.do_best_first = aiuto;
		else{
			GpG.do_best_first = TRUE;
	 		GpG.numrestart = 0;
		}	
...
//	}
	...
	sscanf (argv[++i], "%d", &GpG.inst_duplicate_param);
	...
	sscanf (argv[++i], "%d", &GpG.static_noise);

/**
VALLATI, processo i valori del noise dinamico
**/
      else if (strcmp (argv[i], "-dynoisecoefnum") == 0)
	sscanf (argv[++i], "%f", &GpG.numerator_coef);

      else if (strcmp (argv[i], "-dynoiseTabLen") == 0)
	sscanf (argv[++i], "%f", &GpG.tabu_length_coef);

      else if (strcmp (argv[i], "-inc_re") == 0)
	sscanf (argv[++i], "%f", &GpG.inc_restart);

      else if (strcmp (argv[i], "-uns_vect") == 0)
	sscanf (argv[++i], "%d", &GpG.uns_vect);

      else if (strcmp (argv[i], "-noise_incr") == 0)
	sscanf (argv[++i], "%f", &GpG.noise_increment);
...
**/
      ...
      else if (strcmp (argv[i], "-cputime") == 0)
	{
      ...
      else if (strcmp (argv[i], "-memlimit") == 0)
        {
		// TODO: FAWCETTC PUT MEM LIMIT SETTING HERE.
		long limitInMB;
		sscanf (argv[++i], "%ld", &limitInMB);
		
		struct rlimit limit;
		limit.rlim_cur = limitInMB*1024*1024;
		limit.rlim_max = limitInMB*1024*1024;

		setrlimit(RLIMIT_AS, &limit);
        }
//VALLATI mi serve solo per definire i diversi tipi
      else if (strcmp (argv[i], "-vicinato") == 0)
	sscanf (argv[++i], "%d", &aiuto);
	...
	sscanf (argv[++i], "%d", &GpG.verify_init);

      else if (strcmp (argv[i], "-verifyAf") == 0)
	sscanf (argv[++i], "%d", &GpG.verify_Af);
	...
	**/
	...
	sscanf (argv[++i], "%d", &GpG.relaxed_examination_type);
	...
	**/
	...
	sscanf (argv[++i], "%d", &GpG.relax_list_fact_cost);
       /** 
	   Aggiunta gestione a lista per informazioni raggiungibilita'
	**/
	...
	sscanf (argv[++i], "%d", &GpG.num_elem_neighb_restrict);
      else if (strcmp (argv[i], "-ncut") == 0)
	{
		GpG.number_restrict_neighb = TRUE;
		sscanf (argv[++i], "%d", &GpG.num_elem_neighb_restrict);
	}
      else if (strcmp (argv[i], "-no_cut") == 0)
	{
		sscanf (argv[++i], "%d", &aiuto);
		if (aiuto == 1){
 ...
 //	GpG.number_restrict_neighb = TRUE;
	       ...
	       || strcmp (argv[i], "-notabuplan") == 0)
	{
      ...
      else if (strcmp (argv[i], "-tabu_act") == 0)
	sscanf (argv[++i], "%d", &GpG.tabuplan_act);
      else if (strcmp (argv[i], "-tabu_fct") == 0)
	sscanf (argv[++i], "%d", &GpG.tabuplan_fct);
	...
	sscanf (argv[++i], "%d", &GpG.cri_insertion_add_mutex);
      else if (strcmp (argv[i], "-extended_effects_evaluation") == 0)
	sscanf (argv[++i], "%d", &GpG.extended_effects_evaluation);
      else if (strcmp (argv[i], "-mutex_and_additive_effects") == 0)
	sscanf (argv[++i], "%d", &GpG.mutex_and_additive_effects);
      else if (strcmp (argv[i], "-not_supported_preconds_evaluation") == 0)
	sscanf (argv[++i], "%d", &GpG.supported_preconds_evaluation);
      else if (strcmp (argv[i], "-not_extended_unsupported_facts") == 0)
	sscanf (argv[++i], "%d", &GpG.extended_unsupported_facts);
      else if (strcmp (argv[i], "-extended_unsupported_goals") == 0)
	sscanf (argv[++i], "%d", &GpG.extended_unsupported_goals);
      else if (strcmp (argv[i], "-no_insert_threated_act_in_neighb") == 0)
	sscanf (argv[++i], "%d", &GpG.insert_threated_act_in_neighb);
	...
	sscanf (argv[++i], "%d", &GpG.relaxed_neighborhood_evaluation);
	...
	sscanf (argv[++i], "%d",  &GpG.save_action_cost_list);
	...
	sscanf (argv[++i], "%d",  &GpG.stop_remove_act);
	...
	sscanf (argv[++i], "%d", &GpG.no_mutex_with_additive_effects);
      else if (strcmp (argv[i], "-evaluate_mutex_for_action_remotion") == 0)
	sscanf (argv[++i], "%d", &GpG.evaluate_mutex_for_action_remotion);
