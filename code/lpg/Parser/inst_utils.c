...
int init_bit_table_const(unsigned long int max_size, int *n_bit, int *base, int *bit_row_size) {
   ...
   **
   * bit_pos = posizione del bit pi� significativo di max_size
   * In pratica la tabella ha un numero di righe avente ordine di
   * grandezza pari alla met� dell'ordine di grandezza di max_size
   * (ma come minimo 5)
   **/
...
/*
-----------------------------------------------------------------
	DESCRIPTION	: Verifica se il fatto � booleano
	  ...
	  if ((e->connective == AT_END_CONN) && (e->sons->connective == ATOM)) 
	      ...
	      if (pos == -1)
		{
	     ...
	     /*Effetto additivo At-end*/
	      /* Rimuovo l'azione del vettore gft_conn[...].A solo se il fatto � nelle precondizioni */
	  ...
	  */
	  if ((e->connective == AT_START_CONN) && (e->sons->connective == ATOM))
	      ...
	      if (pos == -1)
		{
	      ...
	      /* Rimuovo l'azione del vettore gft_conn[...].A solo se il fatto � nelle precondizioni at-start */
...
void resize_num_var_vects() {
  ...
  /*
    ...
    non cresce pi� dopo la fase di instanziazione iniziale (inst_pre.c)

    gcomp_var_effects cresce in modo indipendente dagli altri vettori, quindi potrei riallocarlo in un
    ciclo a parte. Lo faccio qui perch� in realt� mantiene sempre lo stesso ordine di grandezza di gcomp_var.
  */
  ...
  if ((gnum_comp_var >= (max_num_value - 1)) || (gnum_comp_var_effects >= (max_num_value - 1))) {
    ...
    /*
      NB !!! Faccio cos� perch� gis_inertial � un bitvector.
      Se facessi come per gli altri vettori, la memset potrebbe resettare blocchi gi�
      parzialmente utilizzati, perch� dovrei necessariamente resettare un numero intero
      di byte.
    */
...
get_index_of_pred (char *arg)
{
  ...
  // verifica se il predicato arg � stato trasformato in NOT-arg
...
void set_cost_and_time_coeffs (void)
{
  ...
  if (gcomp_var[father_cvar].l_operator == MUL_OP)
	...
	if ((gcomp_var[father_cvar].second_op == TOTAL_TIME_FUNCTION_INDEX) && (gcomp_var[gcomp_var[father_cvar].first_op].l_operator == FIX_NUMBER))
	{
...
void create_new_split_precond(int start_ef, int end_ef) {
  ...
  /* 
     Aggiungo un fatto fittizio agli effetti dell'azione start_ef. Questo fatto � 
 ...
 * che erano state saltate perch� contraddittorie
 */
